...
- you win by killing the boss and lose by everyone dying (character dies after being hit ~1-2~ 3 times)
...
- ~character selection will take place one at a time on a character selection screen (image will be greyed out when selected and removed from the selectable ArrayList)~ Character selection will take place on a character selection screen where both players (or the singular player) must lock in their choices before proceeding (by clicking ENTER).
  - change is because this method is better
...
- ~possibly their own classes~
  - does not require its own classes 
- end screen will feature a replay button

bullets have a ~3~ 1 second delay before you can shoot your next one
- ~delay(1000)~
  - using delay would stop the entire program from running until the time has passed
- set up a method that runs when bullet fired is true, decrementing a cooldown variable for the character after every tick ~by 1 every second and setting the bullet fired boolean to false after this reaches 0, resetting the variable to 3. (by creating a variable instead of using delay(3000), we can display the cooldown)~
  - does not require a boolean, we can just directly monitor the cooldown variable and a displayed cooldown was not done in favor of time
...
- cats will not be voiced by mr Ks cats :(
...
- player has to press the jump button 5 times on dead ally to revive ~player must click jump (causing a double jump) upon reaching the constantly floating upwards 'soul' (resembling Cuphead's revive feature)~
  - This method of revival was preferred 
...
|  ~game~ catformers class    |  Stores current game mode ("menu"/"versus"/"boss") + references to players, boss, projectiles, and platforms. Updates/draws all objects depending on mode each frame; responsible for input handling and win/lose conditions.       |
